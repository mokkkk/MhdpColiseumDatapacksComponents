...
# 対象が近い場合、車庫入れ
    execute if entity @s[tag=!Mns.Temp.Valk.MoveToChangePhase] if predicate {"condition":"minecraft:random_chance","chance":0.15} if entity @n[tag=Mns.Target.Valk,distance=..14] run tag @s add Anim.MoveBack
