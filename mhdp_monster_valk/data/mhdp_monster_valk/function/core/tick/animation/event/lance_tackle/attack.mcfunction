#> mhdp_monster_valk:core/tick/animation/event/lance_tackle/attack_l
#
...
# データ読み込み
    data modify storage mhdp_core:temp Damage set from storage mhdp_core:monster_data AttackData[{Uid:1004}].Attacks[{Name:"Tackle"}]
...
# ヒット確認
    # 対プレイヤー
        execute positioned ^ ^1 ^4 as @a[tag=Ply.State.EnableDamage,distance=..3.4] run tag @s add Temp.Hit
    ...
    # 対モンスター
        execute positioned ^ ^1 ^4 as @e[type=slime,tag=Mns.HitBox,tag=!Mns.HitBox.DisableMnsDamage,tag=!Mns.HitBox.Valk,distance=..3.4] run tag @s add Temp.Hit
