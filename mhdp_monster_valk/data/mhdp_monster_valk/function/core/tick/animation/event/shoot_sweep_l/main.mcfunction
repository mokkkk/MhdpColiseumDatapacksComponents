...
#
# アニメーションイベントハンドラ 薙ぎ払い (左)
#
# @within function mhdp_monster_valk:core/tick/animation/event/tick

# 軸合わせ
    execute if score @s aj.shoot_sweep_l.frame matches 2 run function mhdp_monsters:core/util/tick/event/alignment_start.m {TargetTag:"Mns.Target.Valk",Tick:10,MaxRotation:180}
    execute if score @s aj.shoot_sweep_l.frame matches 11 run function mhdp_monsters:core/util/tick/event/alignment_start.m {TargetTag:"Mns.Target.Valk",Tick:10,MaxRotation:180}
    execute if score @s aj.shoot_sweep_l.frame matches 2..20 at @s run function mhdp_monsters:core/util/tick/event/alignment
...
# 攻撃
    execute if score @s aj.shoot_sweep_l.frame matches 49 run function mhdp_monsters:core/util/tick/event/start_attack.m with storage mhdp_core:monster_data AttackData[{Uid:1004}].Attacks[{Name:"Sweep.Left"}]
    ...
    execute if score @s aj.shoot_sweep_l.frame matches 57 run function mhdp_monsters:core/util/tick/event/end_attack
...
# 状態更新
    execute if score @s aj.shoot_sweep_l.frame matches 45 run tag @s add Mns.Valk.State.Attack.Wing.L
    execute if score @s aj.shoot_sweep_l.frame matches 60 run tag @s remove Mns.Valk.State.Attack.Wing.L
