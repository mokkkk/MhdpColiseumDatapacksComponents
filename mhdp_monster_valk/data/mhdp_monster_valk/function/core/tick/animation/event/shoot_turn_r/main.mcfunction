...
# 軸合わせ
    execute if score @s aj.shoot_turn_r.frame matches 2 run function mhdp_monsters:core/util/tick/event/alignment_start.m {TargetTag:"Mns.Target.Valk",Tick:14,MaxRotation:360}
    execute if score @s aj.shoot_turn_r.frame matches 2..15 at @s run function mhdp_monsters:core/util/tick/event/alignment
