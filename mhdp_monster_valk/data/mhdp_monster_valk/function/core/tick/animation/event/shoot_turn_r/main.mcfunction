...
# 軸合わせ
    execute if score @s aj.shoot_turn_r.frame matches 1 run function mhdp_monster_valk:core/tick/animation/event/shoot_turn_r/turn_start
    execute if score @s aj.shoot_turn_r.frame matches 2..15 run function mhdp_monsters:core/util/other/turn_to_target_rotate

# 移動
    execute if score @s aj.shoot_turn_r.frame matches 2..10 if entity @n[tag=Mns.Target.Valk,distance=..7] at @s run tp @s ^ ^ ^-0.3
