...
# 軸合わせ
    execute if score @s aj.shoot_move_start.frame matches 1 run function mhdp_monsters:core/util/tick/event/alignment_start.m {TargetTag:"Mns.MovePos.Valk",Tick:3,MaxRotation:360}
    execute if score @s aj.shoot_move_start.frame matches 2..15 at @s run function mhdp_monsters:core/util/tick/event/alignment
