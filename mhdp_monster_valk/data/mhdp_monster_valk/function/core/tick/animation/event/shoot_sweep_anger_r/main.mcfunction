...
#
# アニメーションイベントハンドラ 薙ぎ払い・怒り時 (右)
#
# @within function mhdp_monster_valk:core/tick/animation/event/tick

# 軸合わせ
    execute if score @s aj.shoot_sweep_anger_r.frame matches 2 run function mhdp_monsters:core/util/tick/event/alignment_start.m {TargetTag:"Mns.Target.Valk",Tick:10,MaxRotation:180}
    execute if score @s aj.shoot_sweep_anger_r.frame matches 11 run function mhdp_monsters:core/util/tick/event/alignment_start.m {TargetTag:"Mns.Target.Valk",Tick:10,MaxRotation:180}
    execute if score @s aj.shoot_sweep_anger_r.frame matches 2..20 at @s run function mhdp_monsters:core/util/tick/event/alignment
...
# 攻撃
    execute if score @s aj.shoot_sweep_anger_r.frame matches 49 run function mhdp_monsters:core/util/tick/event/start_attack.m with storage mhdp_core:monster_data AttackData[{Uid:1004}].Attacks[{Name:"Sweep.Anger.Right"}]
    ...
    execute if score @s aj.shoot_sweep_anger_r.frame matches 57 run function mhdp_monsters:core/util/tick/event/end_attack
