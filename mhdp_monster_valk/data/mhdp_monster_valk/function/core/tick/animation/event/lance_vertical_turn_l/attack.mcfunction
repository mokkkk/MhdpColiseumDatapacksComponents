#> mhdp_monster_valk:core/tick/animation/event/lance_vertical_turn_l/attack
#
...
# ヒット確認
    # 対プレイヤー
        execute as @a[tag=Ply.State.EnableDamage,distance=..4.5] run tag @s add Temp.Hit
        execute as @a[tag=Temp.Hit] run function mhdp_core:player/damage/entity_to_player/main
        tag @a remove Temp.Hit
    # 対モンスター
        execute as @e[type=slime,tag=Mns.HitBox,tag=!Mns.HitBox.DisableMnsDamage,tag=!Mns.HitBox.Valk,distance=..4.5] run tag @s add Temp.Hit
...
# 演出
    ...
    execute positioned ^-0.5 ^-3 ^0.3 facing entity @e[type=marker,tag=Mk.Field.Back,sort=random,limit=1] feet rotated ~ 0 run function mhdp_monsters:core/effect/crack_ground/start
    execute positioned ^ ^1 ^ rotated ~ -90 run function mhdp_monster_valk:core/tick/animation/event/lance_vertical_turn_l/particle_ring
