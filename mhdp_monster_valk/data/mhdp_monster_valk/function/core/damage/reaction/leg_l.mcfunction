#> mhdp_monster_valk:core/damage/reaction/leg_l
#
# 怯みリアクション 左足
#
...
# アニメーション再生 (3回目からダウン)
    execute if entity @s[tag=!Mns.State.IsFlying,tag=!Mns.Temp.IsDamaged] if score @s Mns.Valk.LegL.Damage.Count matches ..2 run function animated_java_valk:valk/animations/lance_damage_body_l/tween {duration:1, to_frame: 0}
    execute if entity @s[tag=!Mns.State.IsFlying,tag=!Mns.Temp.IsDamaged] if score @s Mns.Valk.LegL.Damage.Count matches 3.. run function animated_java_valk:valk/animations/lance_damage_down_l/tween {duration:1, to_frame: 0}
