...
# アニメーション再生 (直前に右ダウン中なら左へ、それ以外は右へ)
    execute if entity @s[tag=animated_java_valk.valk.animation.lance_down_r.playing] run tag @s add Mns.Temp.Right
    execute if entity @s[tag=!Mns.Temp.Right,tag=!Mns.State.IsFlying,tag=!Mns.Temp.IsDamaged] run function animated_java_valk:valk/animations/lance_damage_down_r/tween {duration:1, to_frame: 0}
    execute if entity @s[tag=Mns.Temp.Right,tag=!Mns.State.IsFlying,tag=!Mns.Temp.IsDamaged] run function animated_java_valk:valk/animations/lance_damage_down_l/tween {duration:1, to_frame: 0}
