...
# 部位破壊処理
    execute if entity @s[tag=!Mns.Break.Arm.R] run function mhdp_monster_valk:core/damage/break/arm_r

# 麻痺・ダウン・スタン時はアニメーションを再生しない
    execute unless entity @s[tag=!Mns.State.IsParalysis,tag=!Mns.State.IsDown,tag=!Mns.State.IsStun] run return 0

# アニメーション再生 (3回目からダウン)
    execute if entity @s[tag=!Mns.State.IsFlying,tag=!Mns.Temp.IsDamaged] if score @s Mns.Valk.ArmR.Damage.Count matches ..2 run function animated_java_valk:valk/animations/lance_damage_body_r/tween {duration:1, to_frame: 0}
    execute if entity @s[tag=!Mns.State.IsFlying,tag=!Mns.Temp.IsDamaged] if score @s Mns.Valk.ArmR.Damage.Count matches 3.. run function animated_java_valk:valk/animations/lance_damage_down_r/tween {duration:1, to_frame: 0}
