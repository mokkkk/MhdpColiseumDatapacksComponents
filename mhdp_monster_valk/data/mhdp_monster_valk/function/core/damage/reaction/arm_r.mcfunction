...
# アニメーション再生処理
    ...
    # アニメーション再生
            execute if score @s Mns.Valk.ArmR.Damage.Count matches ..1 run function animated_java:valk_aj/animations/lance_damage_body_r/tween {duration:1, to_frame: 0}
            execute if score @s Mns.Valk.ArmR.Damage.Count matches 2.. run function animated_java:valk_aj/animations/lance_damage_down_r/tween {duration:1, to_frame: 0}
    ...
    # 状態設定
        execute if score @s Mns.Valk.ArmR.Damage.Count matches 2.. run tag @s add Mns.State.IsDown

# 終了
    execute if score @s Mns.Valk.ArmR.Damage.Count matches 2.. run scoreboard players set @s Mns.Valk.ArmR.Damage.Count 0
