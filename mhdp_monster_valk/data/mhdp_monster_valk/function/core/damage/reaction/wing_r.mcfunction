...
# 部位破壊処理
    execute if entity @s[tag=!Mns.Break.Wing.R] run function mhdp_monster_valk:core/damage/break/wing_r

# 麻痺・ダウン・スタン時はアニメーションを再生しない
    execute unless entity @s[tag=!Mns.State.IsParalysis,tag=!Mns.State.IsDown,tag=!Mns.State.IsStun] run return 0

# アニメーション再生
    execute if entity @s[tag=!Mns.State.IsFlying,tag=!Mns.Temp.IsDamaged] run function animated_java_valk:valk/animations/lance_damage_wing_r/tween {duration:1, to_frame: 0}
