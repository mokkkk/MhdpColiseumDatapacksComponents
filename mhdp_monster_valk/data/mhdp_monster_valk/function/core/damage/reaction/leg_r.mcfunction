...
# アニメーション再生 (3回目からダウン)
    execute if entity @s[tag=!Mns.State.IsFlying,tag=!Mns.Temp.IsDamaged] if score @s Mns.Valk.LegR.Damage.Count matches ..2 run function animated_java_valk:valk/animations/lance_damage_body_r/tween {duration:1, to_frame: 0}
    execute if entity @s[tag=!Mns.State.IsFlying,tag=!Mns.Temp.IsDamaged] if score @s Mns.Valk.LegR.Damage.Count matches 3.. run function animated_java_valk:valk/animations/lance_damage_down_r/tween {duration:1, to_frame: 0}
...
# 独自処理
    # ダウン時間設定
        ...
        execute if score @s Mns.Valk.LegR.Damage.Count matches 3.. run scoreboard players set @s Mns.General.DownCount 5
    # 状態設定
        execute if score @s Mns.Valk.LegR.Damage.Count matches 3.. run tag @s add Mns.State.IsDown
