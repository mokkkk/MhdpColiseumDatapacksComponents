...
# 非戦闘時、戦闘に移行
    execute unless score @s Mns.General.Phase matches 2 run function mhdp_monster_valk:core/tick/on_battle/start_ambush
...
# 攻撃プレイヤーのヘイト増加
    scoreboard players add @a[tag=Mns.Candidate.Valk,tag=Temp.Attacker] Mns.Valk.Hate 10
...
# 怯み適用：優先度順 (1被弾で最大1つ。Mns.Temp.IsDamaged が立ったら以降スキップ)
    # 麻痺
    ...
    # 怒り
        execute if entity @s[tag=!Mns.Temp.IsDamaged,tag=Mns.Temp.Damage.Anger,tag=!Mns.State.IsFlying,tag=!animated_java_valk.valk.animation.lance_charge.playing] run function mhdp_monster_valk:core/damage/reaction/anger
