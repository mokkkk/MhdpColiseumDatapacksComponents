...
# アニメーション再生処理
    ...
    # アニメーション再生
        execute if entity @s[tag=!Mns.State.IsParalysis,tag=!Mns.State.IsDown,tag=!Mns.State.IsStun] if entity @s[tag=!Mns.State.IsFlying,tag=!Mns.Temp.IsDamaged] run function animated_java:ranposu/animations/damage/tween {duration:1, to_frame: 0}
