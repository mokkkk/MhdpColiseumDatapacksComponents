...
# データ読み込み
    function mhdp_monsters:core/util/tick/get_attack_data.m {Uid:1001, Name:"MoveClaw"}
    # data modify storage mhdp_core:temp Damage set from storage mhdp_core:monster_data AttackData[{Uid:1001}].Attacks[{Name:"MoveClaw"}]
...
# ヒット確認
    ...
    # 対モンスター
        execute positioned ^ ^1 ^1 as @e[type=slime,tag=Mns.HitBox,tag=!Mns.HitBox.DisableMnsDamage,tag=!Mns.HitBox.Ranposu,distance=..3] run tag @s add Temp.Hit
