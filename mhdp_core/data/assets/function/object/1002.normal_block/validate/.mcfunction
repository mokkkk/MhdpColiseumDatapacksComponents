...
# 設置範囲のエンティティ確認
    execute if data storage api: Arg{Valid:true} align xyz positioned ~0.5 ~0.5 ~0.5 if entity @n[type=shulker,tag=Asset.Build.HitBox,dx=0.01,dy=0.01,dz=0.01] run data modify storage api: Arg.Valid set value false
