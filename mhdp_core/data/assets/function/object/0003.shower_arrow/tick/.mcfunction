#> assets:object/0003.shower_arrow/tick/
#
# 矢のtick処理
#
...
# メイン処理実行
    execute if entity @s[tag=!3.IsHit] at @s run function assets:object/0003.shower_arrow/tick/move
    execute if entity @s[tag=!3.IsHit] at @s run function assets:object/0003.shower_arrow/tick/move

# 攻撃
    execute if entity @s[tag=3.IsHit] at @s run function assets:object/0003.shower_arrow/tick/attack_normal
