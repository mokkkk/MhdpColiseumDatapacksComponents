#> mhdp_monsters:core/util/tick/event/apply_attack_with_entitypos.m
# 
# 汎用処理 攻撃の実行 実行位置となるエンティティを与える
#
# @within function mhdp_monsters:/**
# @input arg Uid モンスターのUID
# @input arg AttackName ストレージ検索に用いる攻撃名
# @input arg EntityPosSelector 実行位置となるエンティティのセレクタ
...
# 対プレイヤー
    ...
    $execute as @a[tag=Temp.Hit] positioned as $(EntityPosSelector) run function api:damage_entity_to_player
...
# 対建築物
    ...
    $execute as @e[type=shulker,tag=Temp.Hit,distance=..30] positioned as $(EntityPosSelector) run function api:damage_entity_to_object
    data remove storage api: Arg
...
# 対モンスター
    ...
    $execute as @e[type=slime,tag=Temp.Hit,distance=..30] positioned as $(EntityPosSelector) run function api:damage_entity_to_entity
    data remove storage api: Arg
