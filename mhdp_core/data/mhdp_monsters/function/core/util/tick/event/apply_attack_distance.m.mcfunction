#> mhdp_monsters:core/util/tick/event/apply_attack_distance.m
# 
# 汎用処理 攻撃の実行
#
...
# @input arg Player_Distance プレイヤー用当たり判定Distance
...
# @input arg Entity_Scale_X エンティティ用当たり判定Distance
...
# 対プレイヤー
    ...
    $execute positioned ^$(Player_Offset_X) ^$(Player_Offset_Y) ^$(Player_Offset_Z) run tag $(Player_Selector) add Temp.Hit
    execute rotated as @s as @a[tag=Temp.Hit] run function api:damage_entity_to_player
...
# 対建築物
    ...
    $execute positioned ^$(Entity_Offset_X) ^$(Entity_Offset_Y) ^$(Entity_Offset_Z) run tag @e[type=shulker,tag=Entity.EnableDamage,distance=..$(Entity_Distance)] add Temp.Hit
    $execute rotated as @s as @e[type=shulker,tag=Temp.Hit,distance=..$(Entity_Distance)] run function api:damage_entity_to_object
    data remove storage api: Arg
...
# 対モンスター
    ...
    $execute positioned ^$(Entity_Offset_X) ^$(Entity_Offset_Y) ^$(Entity_Offset_Z) run tag $(Entity_Selector) add Temp.Hit
    $execute rotated as @s as @e[type=slime,tag=Temp.Hit,distance=..$(Entity_Distance)] run function api:damage_entity_to_entity
    data remove storage api: Arg
    $tag @e[type=slime,tag=Temp.Hit,distance=..$(Entity_Distance)] remove Temp.Hit
