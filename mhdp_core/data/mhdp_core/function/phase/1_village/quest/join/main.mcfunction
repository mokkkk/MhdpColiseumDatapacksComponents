...
# クエスト受注条件に満たない場合、実行しない
    execute unless data storage mhdp_core:game_data {Phase:1} run tellraw @s {"text":"【現在クエスト中です。終了までお待ちください】","color": "red"}
    execute unless data storage mhdp_core:game_data {Phase:1} run return 0
    execute unless score #mhdp_core_loading_quest_id MhdpCore matches 0.. run tellraw @s {"text":"【クエストが受注されていません】","color": "red"}
    execute unless score #mhdp_core_loading_quest_id MhdpCore matches 0.. run return 0

# ホストの場合、実行しない
    execute if entity @s[tag=Ply.State.QuestHost] run tellraw @s {"text":"【既にクエストを受注しています】","color": "red"}
