#> mhdp_core:phase/1_village/villager/weapon_tutorial/short_sword_tec/6_backstep
#
# 武器チュートリアル 片手剣・天ノ型
#
...
# 初期スコア表示
    execute if score @s Vlg.General.Timer matches 2 run scoreboard players display name $mhdp_temp_weapon_tutorial_value Vlg.WeaponTutorial.Text {"text":"バックステップを使用する：残り","color":"green"}
    ...
    execute if score @s Vlg.General.Timer matches 2 run scoreboard players display name $mhdp_temp_weapon_tutorial_value_2 Vlg.WeaponTutorial.Text {"text":"突進斬りを使用する：残り","color":"green"}
...
# スコア設定
    ...
    # スコア減算
        execute if score @s Vlg.General.Timer matches 3.. \
            ...
            if entity @a[tag=Ply.State.IsTraining,team=Team.WeaponTutorial,tag=Wpn.Ss.Tec.Backstep,scores={Wpn.GeneralTimer=2}] run \
    ...
    # チュートリアル完了
        execute if score @s Vlg.General.Timer matches 3.. \
            ...
            scoreboard players display name $mhdp_temp_weapon_tutorial_value Vlg.WeaponTutorial.Text {"text":"バックステップを使用する：","color":"green"}
        execute if score @s Vlg.General.Timer matches 3.. \
            if score $mhdp_temp_weapon_tutorial_value Vlg.WeaponTutorial.Text matches 0 run \
        ...
        execute if score @s Vlg.General.Timer matches 3.. \
            ...
            scoreboard players display name $mhdp_temp_weapon_tutorial_value_2 Vlg.WeaponTutorial.Text {"text":"突進斬りを使用する：","color":"green"}
        execute if score @s Vlg.General.Timer matches 3.. \
            if score $mhdp_temp_weapon_tutorial_value_2 Vlg.WeaponTutorial.Text matches 0 run \
