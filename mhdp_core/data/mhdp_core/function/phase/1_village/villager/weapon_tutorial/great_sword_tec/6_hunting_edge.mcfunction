#> mhdp_core:phase/1_village/villager/weapon_tutorial/great_sword_tec/6_hunting_edge
#
# 武器チュートリアル 大剣・天ノ型
#
...
# メッセージ
    ...
    execute if score @s Vlg.General.Timer matches 210 run tellraw @a[tag=Ply.State.IsTraining,team=Team.WeaponTutorial] [\
        ...
        {"text":"  着地後は、","color": "#00FFC3","bold": false},{"text":"マウス","color": "#00FFC3","bold": false},{"keybind":"key.use","color": "#00FFC3","bold": false},{"text":"長押しで強溜めに移行します。\n","color": "#00FFC3","bold": false}\
    ]

# 初期スコア表示
    execute if score @s Vlg.General.Timer matches 2 run scoreboard players display name $mhdp_temp_weapon_tutorial_value Vlg.WeaponTutorial.Text {"text":"ハンティングエッジを当てる：残り","color":"green"}
    ...
    execute if score @s Vlg.General.Timer matches 2 run scoreboard players display name $mhdp_temp_weapon_tutorial_value_2 Vlg.WeaponTutorial.Text {"text":"最大まで溜めた空中溜め斬りを使用する：残り","color":"green"}
...
# スコア設定
    ...
    # スコア減算
        execute if score @s Vlg.General.Timer matches 3.. \
            ...
            if entity @a[tag=Ply.State.IsTraining,team=Team.WeaponTutorial,tag=Wpn.Gs.Tec.Charge.Edge,scores={Wpn.GeneralTimer=2}] run \
        ...
        execute if score @s Vlg.General.Timer matches 3.. \
            ...
            if entity @a[tag=Ply.State.IsTraining,team=Team.WeaponTutorial,tag=Wpn.Gs.Tec.ChargeSlash.Edge,scores={Wpn.GeneralTimer=2,Wpn.Gs.ChargeCount=3..}] run \
    ...
    # チュートリアル完了
        execute if score @s Vlg.General.Timer matches 3.. \
            ...
            scoreboard players display name $mhdp_temp_weapon_tutorial_value Vlg.WeaponTutorial.Text {"text":"ハンティングエッジを当てる：","color":"green"}
        execute if score @s Vlg.General.Timer matches 3.. \
            if score $mhdp_temp_weapon_tutorial_value Vlg.WeaponTutorial.Text matches 0 run \
        ...
        execute if score @s Vlg.General.Timer matches 3.. \
            ...
            scoreboard players display name $mhdp_temp_weapon_tutorial_value_2 Vlg.WeaponTutorial.Text {"text":"最大まで溜めた空中溜め斬りを使用する：","color":"green"}
        execute if score @s Vlg.General.Timer matches 3.. \
            if score $mhdp_temp_weapon_tutorial_value_2 Vlg.WeaponTutorial.Text matches 0 run \
