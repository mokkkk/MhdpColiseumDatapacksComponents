...
# アニメーション演出
    ...
    execute if entity @s[nbt={OnGround:1b}] if score @s Wpn.AnimationTimer matches 4 rotated ~ 0 run function mhdp_items:weapons/great_sword/type_tec/2_chargeattack/effect_on_ground
...
# 遷移
    # 右クリックを離した場合、溜めに移行
        execute if entity @s[tag=Ply.Ope.IsUsingEnderEye] if score @s Wpn.GeneralTimer matches 20.. run function mhdp_items:weapons/great_sword/type_tec/2_chargeattack/change_to_charge
