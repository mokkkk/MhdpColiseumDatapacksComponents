...
# ヒットストップ
    execute if score @s Wpn.Gs.ChargeCount matches ..2 if entity @n[tag=Temp.Victim] run scoreboard players set @s Wpn.HitStopTimer 3
    execute if score @s Wpn.Gs.ChargeCount matches 3.. if entity @n[tag=Temp.Victim] run scoreboard players set @s Wpn.HitStopTimer 7
    execute if entity @e[tag=Temp.Victim] run tp @s @s

# 攻撃
    execute if score @s Wpn.Gs.ChargeCount matches ..1 run data modify storage mhdp_core:temp Damage set from storage mhdp_core:game_data WeaponAttackData.GreatSword.Normal.Cross.1.2
    execute if score @s Wpn.Gs.ChargeCount matches 2 run data modify storage mhdp_core:temp Damage set from storage mhdp_core:game_data WeaponAttackData.GreatSword.Normal.Cross.2.2
    execute if score @s Wpn.Gs.ChargeCount matches 3.. run data modify storage mhdp_core:temp Damage set from storage mhdp_core:game_data WeaponAttackData.GreatSword.Normal.Cross.3.2
