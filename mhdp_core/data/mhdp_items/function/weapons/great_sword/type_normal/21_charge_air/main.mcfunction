#> mhdp_items:weapons/great_sword/type_normal/21_charge_air/main
#
# 空中溜め メイン処理
#
...
# 操作表示
    execute if score @s Wpn.GeneralTimer matches 1 run function mhdp_items:core/util/item_modify_custom_name {Name:"空中溜め"}
...
# 移動制限
    ...
    execute if score @s Wpn.GeneralTimer matches 15 run function api:weapon_operation/attribute_moveslow
    execute if score @s Wpn.GeneralTimer matches 1 run tag @s add Ply.Weapon.NoMoveJump

# 溜め
    scoreboard players add @s Wpn.Gs.ChargeTimer 12
...
# 遷移
    # 着地時、溜め斬りに移行
        execute if entity @s[tag=!Ply.Ope.IsAir] if score @s Wpn.GeneralTimer matches 10.. run function mhdp_items:weapons/great_sword/type_normal/21_charge_air/change_to_chargeattack
    # 右クリックを離した場合、溜め斬りに移行
        execute if entity @s[tag=!Ply.Ope.IsUsingEnderEye] if score @s Wpn.GeneralTimer matches 10.. run function mhdp_items:weapons/great_sword/type_normal/21_charge_air/change_to_chargeattack
