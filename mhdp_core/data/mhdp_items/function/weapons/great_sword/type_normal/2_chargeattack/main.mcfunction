...
# 遷移
    # Ctrl+右クリック長押し時、震怒竜怨斬に移行
        execute if entity @s[tag=Ply.Ope.IsUsingEnderEye,tag=Ply.Ope.IsSneaking,tag=Ply.Ope.IsKeySprint] if score @s Wpn.GeneralTimer matches 19.. if score @s Ply.Stats.Arts.2 >= @s Ply.Stats.Arts.2.Max run function mhdp_items:weapons/great_sword/type_normal/4_guard/change_to_rage
