#> mhdp_items:weapons/great_sword/type_normal/10_hunting_edge/main
#
# ハンティングエッジ メイン処理
#
...
# 操作表示
    execute if score @s Wpn.GeneralTimer matches 1 run function mhdp_items:core/util/item_modify_custom_name {Name:"ハンティングエッジ"}
...
# 遷移
    # ヒットした場合、空中溜めに移行
        execute if entity @s[tag=Ply.Flag.Hit] if score @s Wpn.GeneralTimer matches 5 run function mhdp_items:weapons/great_sword/type_normal/10_hunting_edge/change_to_charge

# 終了
    execute if score @s Wpn.GeneralTimer matches 12.. run function mhdp_items:weapons/great_sword/type_normal/10_hunting_edge/end
