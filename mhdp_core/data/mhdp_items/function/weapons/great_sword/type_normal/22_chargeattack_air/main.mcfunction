#> mhdp_items:weapons/great_sword/type_normal/22_chargeattack_air/main
#
# 空中溜め斬り メイン処理
#
...
# 攻撃
    execute if score @s Wpn.GeneralTimer matches 3 run function mhdp_items:weapons/great_sword/type_normal/22_chargeattack_air/attack
...
# 遷移
    # スニーク+ジャンプ：狩技・強化納刀に移行
        execute if entity @s[tag=Ply.Ope.Buffering.A] if score @s Wpn.GeneralTimer matches 16.. run function mhdp_items:weapons/great_sword/type_normal/17_power_sheathe/start
    # ジャンプ回避
...
# 終了
    execute if score @s Wpn.GeneralTimer matches 26.. run function mhdp_items:weapons/great_sword/type_normal/22_chargeattack_air/end
