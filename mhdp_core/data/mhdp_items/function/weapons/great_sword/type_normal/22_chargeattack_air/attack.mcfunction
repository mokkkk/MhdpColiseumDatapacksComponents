#> mhdp_items:weapons/great_sword/type_normal/22_chargeattack_air/attack
#
# 空中溜め斬り 攻撃判定
#
...
# 攻撃
    execute if score @s Wpn.Gs.ChargeCount matches ..1 run data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.GreatSword.Normal.ChargeSlash.Ride.1
    execute if score @s Wpn.Gs.ChargeCount matches 2 run data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.GreatSword.Normal.ChargeSlash.Ride.2
    execute if score @s Wpn.Gs.ChargeCount matches 3.. run data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.GreatSword.Normal.ChargeSlash.Ride.3
