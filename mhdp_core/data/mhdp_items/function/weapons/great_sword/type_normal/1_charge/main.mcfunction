...
# 遷移
    ...
    # S + スニーク、威姿呵成の構えに移行
        execute if entity @s[tag=Ply.Ope.IsSneaking,tag=Ply.Ope.StartKeyBack,tag=Ply.Ope.IsUsingEnderEye] if score @s Ply.Stats.Arts.1 >= @s Ply.Stats.Arts.1.Max if score @s Wpn.GeneralTimer matches 8.. run function mhdp_items:weapons/great_sword/type_normal/19_counter_stance/start
        execute if entity @s[tag=Ply.Ope.StartSneak,tag=Ply.Ope.IsKeyBack,tag=Ply.Ope.IsUsingEnderEye] if score @s Ply.Stats.Arts.1 >= @s Ply.Stats.Arts.1.Max if score @s Wpn.GeneralTimer matches 8.. run function mhdp_items:weapons/great_sword/type_normal/19_counter_stance/start
        execute if entity @s[tag=Ply.Ope.IsSneaking,tag=Ply.Ope.StartKeySprint,tag=Ply.Ope.IsUsingEnderEye] if score @s Ply.Stats.Arts.1 >= @s Ply.Stats.Arts.1.Max if score @s Wpn.GeneralTimer matches 8.. run function mhdp_items:weapons/great_sword/type_normal/19_counter_stance/start
        execute if entity @s[tag=Ply.Ope.StartSneak,tag=Ply.Ope.IsKeySprint,tag=Ply.Ope.IsUsingEnderEye] if score @s Ply.Stats.Arts.1 >= @s Ply.Stats.Arts.1.Max if score @s Wpn.GeneralTimer matches 8.. run function mhdp_items:weapons/great_sword/type_normal/19_counter_stance/start
