#> mhdp_items:weapons/great_sword/type_tec/21_charge_air/main
#
# 空中溜め メイン処理
#
...
# 遷移
    # 着地時、溜め斬りに移行
        execute if entity @s[tag=!Ply.Ope.IsAir] if score @s Wpn.GeneralTimer matches 10.. run function mhdp_items:weapons/great_sword/type_tec/21_charge_air/change_to_chargeattack
    # 右クリックを離した場合、溜め斬りに移行
        execute if entity @s[tag=!Ply.Ope.IsUsingEnderEye] if score @s Wpn.GeneralTimer matches 10.. run function mhdp_items:weapons/great_sword/type_tec/21_charge_air/change_to_chargeattack
