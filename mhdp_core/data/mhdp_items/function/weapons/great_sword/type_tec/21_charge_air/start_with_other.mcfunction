#> mhdp_items:weapons/great_sword/type_tec/21_charge_air/start_with_other
#
# 空中溜め 開始
#
# @within function mhdp_items:weapons/great_sword/type_tec/10_hunting_edge/change_to_charge

# 前処理
    function mhdp_items:weapons/great_sword/util/before_attack

# タグ付与
    tag @s add Wpn.Gs.Tec.Charge.Air
    tag @s remove Ply.Weapon.NoOpe
    tag @s add Ply.Ope.IsAir

# その他初期化
    scoreboard players set @s Wpn.Gs.ChargeTimer 0
    scoreboard players set @s Wpn.Gs.ChargeCount 0
    scoreboard players set @s Wpn.Gs.ChargeSlashCount 0

# 移動
    execute at @s rotated ~ -90 run function api:weapon_operation/use_player_motion.m {Strength:6000, IsForce:true, IsAdjust:false}

# say 武器操作：大剣・空中溜め