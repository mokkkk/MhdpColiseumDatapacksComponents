...
# 遷移  
    # 右クリック長押し時、相殺斬り上げ・溜めに移行
        execute if entity @s[tag=Ply.Ope.IsSneaking,tag=Ply.Ope.IsUsingEnderEye,tag=!Ply.Ope.IsKeySprint] if score @s Wpn.GeneralTimer matches 10.. unless score @s Wpn.GuardStopTimer matches 1.. run function mhdp_items:weapons/great_sword/type_tec/4_guard/change_to_upper_charge
    # Ctrl+左クリック時、威姿呵成の構えに移行
        execute if entity @s[tag=Ply.Ope.IsSneaking,tag=Ply.Ope.StartLeftClick,tag=Ply.Ope.IsKeySprint] if score @s Wpn.GeneralTimer matches 10.. unless score @s Wpn.GuardStopTimer matches 1.. if score @s Ply.Stats.Arts.1 >= @s Ply.Stats.Arts.1.Max run function mhdp_items:weapons/great_sword/type_tec/4_guard/change_to_stance
    # Ctrl+右クリック長押し時、震怒竜怨斬に移行
        execute if entity @s[tag=Ply.Ope.IsSneaking,tag=Ply.Ope.IsUsingEnderEye,tag=Ply.Ope.IsKeySprint] if score @s Wpn.GeneralTimer matches 10.. unless score @s Wpn.GuardStopTimer matches 1.. if score @s Ply.Stats.Arts.2 >= @s Ply.Stats.Arts.2.Max run function mhdp_items:weapons/great_sword/type_tec/4_guard/change_to_rage
