...
# 遷移  
    ...
    # Ctrl+右クリック長押し時、相殺斬り上げ・溜めに移行
        execute if entity @s[tag=Ply.Ope.IsSneaking,tag=Ply.Ope.IsUsingEnderEye,tag=Ply.Ope.IsKeySprint] if score @s Wpn.GeneralTimer matches 10.. unless score @s Wpn.GuardStopTimer matches 1.. run function mhdp_items:weapons/great_sword/type_tec/4_guard/change_to_rage
