...
# 遷移
    ...
    # 威姿呵成の構え：Sneak+後ろ移動 or Sneak+Ctrl
        execute if entity @s[tag=Ply.Ope.IsSneaking,tag=Ply.Ope.StartKeySprint,tag=Ply.Ope.IsUsingEnderEye] if score @s Ply.Stats.Arts.2 >= @s Ply.Stats.Arts.2.Max if score @s Wpn.GeneralTimer matches 22.. run function mhdp_items:weapons/great_sword/type_tec/19_counter_stance/start
        execute if entity @s[tag=Ply.Ope.StartSneak,tag=Ply.Ope.IsKeySprint,tag=Ply.Ope.IsUsingEnderEye] if score @s Ply.Stats.Arts.2 >= @s Ply.Stats.Arts.2.Max if score @s Wpn.GeneralTimer matches 22.. run function mhdp_items:weapons/great_sword/type_tec/19_counter_stance/start
