#> mhdp_items:weapons/great_sword/type_tec/10_hunting_edge/attack
#
# ハンティングエッジ 攻撃判定
#
...
# 命中判定
    execute anchored eyes positioned ^ ^ ^1 run tag @e[type=slime,tag=Mns.HitBox,distance=..2.5] add Temp.Hit
    execute anchored eyes positioned ^ ^ ^2 run tag @e[type=slime,tag=Mns.HitBox,distance=..2.5] add Temp.Hit
...
# ヒットストップ    
    execute if entity @e[tag=Temp.Victim] run scoreboard players set @s Wpn.HitStopTimer 2
...
# 攻撃
    ...
    data modify storage mhdp_core:temp Arg.VfxRotation set value 0
