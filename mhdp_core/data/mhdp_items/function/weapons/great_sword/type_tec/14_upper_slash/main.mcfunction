...
# 攻撃
    execute if entity @s[tag=!Ply.Flag.Hit] if score @s Wpn.GeneralTimer matches 3..4 run function mhdp_items:weapons/great_sword/type_tec/14_upper_slash/attack
...
# アーマー
    execute if score @s Wpn.GeneralTimer matches 3 run tag @s add Ply.Weapon.Armor.Hyper
    execute if entity @s[tag=!Ply.Flag.CounterSuccess] if score @s Wpn.GeneralTimer matches 4 run tag @s remove Ply.Weapon.Armor.Hyper
...
# 移動
    ...
    execute if entity @s[tag=Ply.Flag.CounterSuccess] if score @s Wpn.AnimationTimer matches 6 rotated ~180 -10 run function api:weapon_operation/use_player_motion.m {Strength:9000, IsForce:false, IsAdjust:false}
