#> mhdp_items:weapons/bow/type_normal/9_equip_bottle/main
#
# ビン装填 メイン処理
#
...
# アニメーション演出
    ...
    execute if score @s Wpn.AnimationTimer matches 14 run playsound block.brewing_stand.brew master @s[tag=!Ply.State.IsSilent] ~ ~ ~ 2 1
    execute if score @s Wpn.AnimationTimer matches 1 run function mhdp_items:weapons/bow/type_normal/9_equip_bottle/animation_0

# ビン装填
    execute if score @s Wpn.AnimationTimer matches 14 run function mhdp_items:weapons/bow/type_normal/9_equip_bottle/equip

# 終了
    execute if entity @s[tag=Wpn.Bw.Normal.Equip] if score @s Wpn.GeneralTimer matches 15.. run function mhdp_items:weapons/bow/type_normal/9_equip_bottle/end
