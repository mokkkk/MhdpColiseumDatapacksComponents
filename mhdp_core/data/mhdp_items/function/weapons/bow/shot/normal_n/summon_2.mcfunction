...
# 召喚
    ...
    data modify storage api: Arg.Override.ChargeCount set value 2
...
# 召喚
    ...
    execute positioned ^0.16 ^0.1 ^ rotated ~-0.3 ~-0.55 run function api:object/summon.m {ObjectId:1}
# 召喚
    execute store result storage api: Arg.Override.PlyUid int 1 run scoreboard players get @s Ply.Uid
    data modify storage api: Arg.Override.ChargeCount set value 2
    data modify storage api: Arg.Override.IsTec set value false
    ...
    execute positioned ^-0.15 ^0.15 ^ rotated ~0.2 ~-0.6 run function api:object/summon.m {ObjectId:1}