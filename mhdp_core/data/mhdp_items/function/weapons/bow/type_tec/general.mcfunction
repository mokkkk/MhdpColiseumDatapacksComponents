...
# 導ノ矢制限時間処理
    ...
    execute if entity @s[tag=Ply.Weapon.Bow.IsHorming] if score @s Wpn.Bw.HormingTimer matches ..0 run scoreboard players reset @s Wpn.Bw.HormingTarget
    execute if entity @s[tag=Ply.Weapon.Bow.IsHorming] if score @s Wpn.Bw.HormingTimer matches ..0 run tag @s remove Ply.Weapon.Bow.IsHorming
