...
# 操作表示
    execute if score @s Wpn.GeneralTimer matches 1 run function mhdp_items:core/util/item_modify_custom_name {Name:"射撃（最大溜め）"}
...
# 先行入力
    ...
    execute if entity @s[tag=Ply.Ope.StartLeftClick] if score @s Wpn.GeneralTimer matches 1..21 run function mhdp_items:core/buffering/b
    execute if entity @s[tag=Ply.Ope.IsSneaking,tag=Ply.Ope.IsUsingEnderEye] if score @s Wpn.GeneralTimer matches 1.. run function mhdp_items:core/buffering/d
...
# 遷移
    ...
    # ジャンプ：身躱し射法
        ...
        execute if entity @s[tag=Ply.Ope.Buffering.Arts1] if score @s Wpn.GeneralTimer matches 8..21 if score @s Ply.Stats.Arts.1 >= @s Ply.Stats.Arts.1.Max run function mhdp_items:weapons/bow/type_tec/11_moving_shot/start
    # スニーク+右クリック：剛射に移行
        execute if entity @s[tag=Ply.Ope.Buffering.B] if score @s Wpn.GeneralTimer matches 8..21 run function mhdp_items:weapons/bow/type_tec/5_shot_power/start
...
# 終了
    execute if entity @s[tag=Wpn.Bw.Tec.Shot.Max,tag=!Ply.Ope.IsUsingEnderEye] if score @s Wpn.GeneralTimer matches 22..35 run function mhdp_items:weapons/bow/type_tec/3_shot_max/end
    execute if entity @s[tag=Wpn.Bw.Tec.Shot.Max] if score @s Wpn.GeneralTimer matches 36.. run function mhdp_items:weapons/bow/type_tec/3_shot_max/end
