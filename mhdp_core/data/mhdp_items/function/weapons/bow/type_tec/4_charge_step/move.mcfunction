...
# 移動
    ...
    # 後ろに移動した場合、移動距離を短くする
        execute if score @n[type=marker,tag=Mk.Temp.PlayerPos.0] MhdpCore matches 1.. positioned as @s rotated as @n[type=marker,tag=Mk.Temp.PlayerPos.0] positioned ^ ^ ^1 rotated as @s rotated ~ 0 positioned ^-0.1 ^ ^1 if entity @s[distance=..1.2] run scoreboard players set $strength player_motion.api.launch 8000
    ...
    execute unless score @n[type=marker,tag=Mk.Temp.PlayerPos.0] MhdpCore matches 1.. rotated ~ 0 run function player_motion:api/launch_looking
