#> mhdp_items:weapons/short_sword/type_tec/25_screw_thrust_spear/attack
#
# スクリュースラスト 攻撃判定
#
...
# 攻撃
    execute if score @s Wpn.Lc.ChargeCount matches ..1 run data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.Lance.Tec.ScrewThrust.1
    execute if score @s Wpn.Lc.ChargeCount matches 2 run data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.Lance.Tec.ScrewThrust.2
    execute if score @s Wpn.Lc.ChargeCount matches 3.. run data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.Lance.Tec.ScrewThrust.3
