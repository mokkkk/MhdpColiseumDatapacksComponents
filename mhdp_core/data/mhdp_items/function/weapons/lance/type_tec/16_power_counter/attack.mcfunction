#> mhdp_items:weapons/short_sword/type_tec/16_power_counter/attack
#
# カウンター突き 攻撃判定
#
...
# 攻撃
    execute if score @s Wpn.Lc.ChargeCount matches ..0 run data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.Lance.Tec.CounterSpear.C1
    execute if score @s Wpn.Lc.ChargeCount matches 1 run data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.Lance.Tec.CounterSpear.C2
    execute if score @s Wpn.Lc.ChargeCount matches 2.. run data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.Lance.Tec.CounterSpear.C3
