...
# 遷移  
    ...
    # 左クリック：突進に移行
        execute if entity @s[tag=Ply.Ope.StartLeftClick] if score @s Wpn.GeneralTimer matches 3.. unless score @s Wpn.GuardStopTimer matches 1.. run function mhdp_items:weapons/lance/type_tec/6_guard/change_to_dash
    # 右クリック：溜めカウンターに移行
