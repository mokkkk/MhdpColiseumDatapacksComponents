...
# 操作表示
    execute if score @s Wpn.GeneralTimer matches 1 run function mhdp_items:core/util/item_modify_custom_name {Name:"weapon.lance.action.spear_3"}
...
# 先行入力
    ...
    execute if entity @s[tag=Ply.Ope.StartLeftClick,tag=Ply.Ope.IsSneaking] if score @s Wpn.GeneralTimer matches 3..21 run function mhdp_items:core/buffering/d
...
# 遷移
    # 左クリック：3連突きに移行
        execute if entity @s[tag=Ply.Ope.Buffering.A] if score @s Wpn.GeneralTimer matches 13.. run function mhdp_items:weapons/lance/type_tec/9_triple_spear/start
    # スニーク+ジャンプ：ガードダッシュに移行
        execute if entity @s[tag=Ply.Ope.Buffering.B] if score @s Wpn.GeneralTimer matches 13.. run function mhdp_items:weapons/lance/type_tec/7_guard_dash/start
    # スニーク+右クリック：溜めに移行
        execute if entity @s[tag=Ply.Ope.Buffering.C] if score @s Wpn.GeneralTimer matches 13.. run function mhdp_items:weapons/lance/type_tec/11_charge_counter/start
    # スニーク+左クリック：突進に移行
        execute if entity @s[tag=Ply.Ope.Buffering.D] if score @s Wpn.GeneralTimer matches 13.. run function mhdp_items:weapons/lance/type_tec/18_dash/start
    # ジャンプ：ステップに移行
