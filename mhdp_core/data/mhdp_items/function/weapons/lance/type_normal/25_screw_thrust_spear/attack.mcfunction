#> mhdp_items:weapons/short_sword/type_normal/25_screw_thrust_spear/attack
#
# スクリュースラスト 攻撃判定
#
...
# ヒットストップ    
    execute if entity @n[tag=Temp.Victim] run scoreboard players set @s Wpn.HitStopTimer 4

# 攻撃
    execute if score @s Wpn.Lc.ChargeCount matches ..1 run data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.Lance.Normal.ScrewThrust.1
    execute if score @s Wpn.Lc.ChargeCount matches 2 run data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.Lance.Normal.ScrewThrust.2
    execute if score @s Wpn.Lc.ChargeCount matches 3.. run data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.Lance.Normal.ScrewThrust.3
