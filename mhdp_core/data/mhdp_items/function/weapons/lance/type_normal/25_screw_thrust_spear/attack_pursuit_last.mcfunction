#> mhdp_items:weapons/short_sword/type_normal/25_screw_thrust_spear/attack_pursuit_last
#
# スクリュースラスト 攻撃判定
#
...
# 命中判定
    ...
    execute anchored eyes positioned ^ ^ ^ positioned ~-0.5 ~-0.5 ~-0.5 run tag @e[type=slime,tag=Mns.HitBox,dx=1,dy=1,dz=1] add Temp.Hit
    execute anchored eyes positioned ^ ^ ^1 positioned ~-0.5 ~-0.5 ~-0.5 run tag @e[type=slime,tag=Mns.HitBox,dx=1,dy=1,dz=1] add Temp.Hit
    execute anchored eyes positioned ^ ^ ^2 positioned ~-0.5 ~-0.5 ~-0.5 run tag @e[type=slime,tag=Mns.HitBox,dx=1,dy=1,dz=1] add Temp.Hit
...
# 攻撃
    data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.Lance.Normal.ScrewThrust.Last
