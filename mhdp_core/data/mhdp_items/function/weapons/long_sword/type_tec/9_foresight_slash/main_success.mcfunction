...
# 遷移
    ...
    # 右クリック：気刃斬り大回転斬りに移行
        execute if entity @s[tag=Ply.Ope.Buffering.B] if score @s Wpn.GeneralTimer matches 13.. if score @s Wpn.Ls.SpiritGauge matches 200.. run function mhdp_items:weapons/long_sword/type_tec/7_spirit_spin/start
    # スニーク+ジャンプ：特殊納刀に移行
