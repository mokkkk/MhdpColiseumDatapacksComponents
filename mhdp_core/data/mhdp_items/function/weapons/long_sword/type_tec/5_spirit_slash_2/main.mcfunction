...
# 遷移
    ...
    # 右クリック：気刃斬り3に移行
        execute if entity @s[tag=Ply.Ope.Buffering.B] if score @s Wpn.GeneralTimer matches 13.. if score @s Wpn.Ls.SpiritGauge matches 150.. run function mhdp_items:weapons/long_sword/type_tec/6_spirit_slash_3/start
    # スニーク左クリック：移動斬りに移行
