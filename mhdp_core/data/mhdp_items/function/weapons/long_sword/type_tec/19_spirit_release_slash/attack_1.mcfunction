...
# 攻撃
    data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.LongSword.Tec.SpiritRelease.2
...
# 演出用VFXオブジェクト召喚
    # データ設定
        ...
        data modify storage api: Arg.Override.Rotation set value 0.9
    ...
    # データ設定
        data modify storage api: Arg.Override.Tag set value "slash_strong"
        data modify storage api: Arg.Override.IsCounter set value false
        data modify storage api: Arg.Override.Rotation set value 1.8
        data modify storage api: Arg.Override.Scale set value [3f,12f,3f]
    # 召喚
