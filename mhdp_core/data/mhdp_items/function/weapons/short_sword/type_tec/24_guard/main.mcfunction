...
# 遷移  
    # 移動せず右クリック：ガード斬りに移行
        execute if entity @s[tag=Ply.Ope.StartUsingEnderEye,tag=!Ply.Ope.IsKeyForward,tag=!Ply.Ope.IsKeyBack] if score @s Wpn.GeneralTimer matches 3.. unless score @s Wpn.GuardStopTimer matches 1.. run function mhdp_items:weapons/short_sword/type_tec/24_guard/change_to_guardslash
    # 前移動+右クリック：盾攻撃に移行
        execute if entity @s[tag=Ply.Ope.StartUsingEnderEye,tag=Ply.Ope.IsKeyForward,tag=!Ply.Ope.IsKeyBack] if score @s Wpn.GeneralTimer matches 3.. unless score @s Wpn.GuardStopTimer matches 1.. run function mhdp_items:weapons/short_sword/type_tec/24_guard/change_to_bash
    # ジャンプ：バックステップに移行
