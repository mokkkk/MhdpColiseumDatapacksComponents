...
# 攻撃
    execute if entity @s[tag=!Ply.Flag.Just.Success] run data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.ShortSword.Tec.Just.2.2
    execute if entity @s[tag=Ply.Flag.Just.Success] run data modify storage api: Arg set from storage mhdp_core:game_data WeaponAttackData.ShortSword.Tec.Just.2.2.S
