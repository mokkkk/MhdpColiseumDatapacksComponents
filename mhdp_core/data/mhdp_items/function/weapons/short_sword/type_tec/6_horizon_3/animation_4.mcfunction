#> mhdp_items:weapons/short_sword/type_tec/10_spin/animation_4
#
# 回転斬り アニメーション処理
#
# @within function mhdp_items:weapons/short_sword/type_tec/10_spin/main

# # メイン
#     # データ設定
#         data modify storage mhdp_core:temp Args.IsDrawing set value true
#         scoreboard players set #mhdp_arg_cmd_offset MhdpCore 28
#         data modify storage mhdp_core:temp Args.Slot set value "mainhand"
#     # アニメーション
#         function mhdp_items:core/util/item_modify_animation

# アニメーション
    function api:weapon/animation.m {Slot:"mainhand", Cmd:"horizon_3_1"}
    function api:weapon/animation.m {Slot:"offhand", Cmd:"spin_2_sub"}

