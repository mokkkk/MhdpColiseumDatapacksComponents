#> mhdp_items:weapons/short_sword/type_tec/5_horizon_2/attack
#
# 水平斬りコンボ2 攻撃判定
#
...
# 命中判定
    ...
    execute anchored eyes positioned ^ ^ ^2 positioned ~-0.6 ~-0.6 ~-0.6 run tag @e[type=slime,tag=Mns.HitBox,dx=1,dy=1,dz=1] add Temp.Hit
...
# 攻撃
    data modify storage mhdp_core:temp Damage set from storage mhdp_core:game_data WeaponAttackData.ShortSword.Tec.Horizon.2
