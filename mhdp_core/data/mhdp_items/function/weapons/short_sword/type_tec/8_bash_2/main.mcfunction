...
# 操作表示
    execute if score @s Wpn.GeneralTimer matches 1 run function mhdp_items:core/util/item_modify_custom_name {Name:"weapon.short_sword.action.shield_combo_2"}
...
# アニメーション演出
    ...
    execute if score @s Wpn.AnimationTimer matches 1 run function mhdp_items:weapons/short_sword/type_tec/8_bash_2/animation_0
    ...
    execute if score @s Wpn.AnimationTimer matches 6 run function mhdp_items:weapons/short_sword/type_tec/8_bash_2/animation_4
...
# 移動
    ...
    execute if score @s Wpn.GeneralTimer matches 3 rotated ~ 0 run function api:weapon_operation/use_player_motion.m {Strength:1200, IsForce:false, IsAdjust:true}
...
# 遷移
    ...
    # 右クリック：バックナックルに移行
        execute if entity @s[tag=Ply.Ope.Buffering.A] if score @s Wpn.GeneralTimer matches 9.. run function mhdp_items:weapons/short_sword/type_tec/9_bash_3/start
    # 同時押し：回転斬りに移行
