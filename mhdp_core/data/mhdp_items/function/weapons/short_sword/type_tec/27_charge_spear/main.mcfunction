...
# 攻撃
    execute if entity @s[tag=!Ply.Flag.Hit] if score @s Wpn.GeneralTimer matches 25..30 run function mhdp_items:weapons/short_sword/type_tec/27_charge_spear/attack_move
...
# 先行入力
    ...
    execute if entity @s[tag=Ply.Ope.StartDoubleJump] if score @s Wpn.GeneralTimer matches 3..44 run function mhdp_items:core/buffering/jump
