...
# 遷移
    # 左クリック：フォールバッシュに移行
        execute if entity @s[tag=Ply.Ope.StartLeftClick] if score @s Wpn.GeneralTimer matches 7.. run function mhdp_items:weapons/short_sword/type_tec/22_fall/start
    # 右クリック：フォールバッシュに移行
