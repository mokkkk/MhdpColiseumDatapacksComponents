...
# アニメーション演出
    ...
    execute if score @s Wpn.AnimationTimer matches 4 run function mhdp_items:weapons/short_sword/type_tec/2_normal_2/animation_2
...
# 遷移
    # 右クリック短押し：通常コンボ2に移行
        # execute if entity @s[tag=Ply.Ope.Buffering.A] if score @s Wpn.GeneralTimer matches 11.. run function mhdp_items:weapons/short_sword/type_tec/2_normal_2/change_to_normal_2

# 終了
    execute if score @s Wpn.GeneralTimer matches 12.. run function mhdp_items:weapons/short_sword/type_tec/2_normal_2/end
