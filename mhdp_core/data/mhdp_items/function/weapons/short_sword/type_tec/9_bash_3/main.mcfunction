...
# 操作表示
    execute if score @s Wpn.GeneralTimer matches 1 run function mhdp_items:core/util/item_modify_custom_name {Name:"weapon.short_sword.action.shield_combo_3"}
...
# 移動
    ...
    # execute if score @s Wpn.GeneralTimer matches 7 rotated ~ -10 run function api:weapon_operation/use_player_motion.m {Strength:2000, IsForce:false, IsAdjust:true}
...
# 先行入力
    ...
    execute if entity @s[tag=Ply.Ope.StartKeyJump,tag=Ply.Ope.IsKeyBack,tag=!Ply.Ope.IsKeySprint] if score @s Wpn.GeneralTimer matches 2..21 run function mhdp_items:core/buffering/g
...
# 遷移
    ...
    # スニーク+ジャンプ時：バックステップに移行
        execute if entity @s[tag=Ply.Ope.Buffering.G] if score @s Wpn.GeneralTimer matches 14.. run function mhdp_items:weapons/short_sword/type_tec/12_backstep/start
    # 同時押し：旋刈りに移行
