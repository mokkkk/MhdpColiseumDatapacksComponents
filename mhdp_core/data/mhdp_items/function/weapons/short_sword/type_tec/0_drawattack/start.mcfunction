...
# 通常：突進斬りに移行
    execute if entity @s[tag=!Wpn.Ss.Tec.FallSpear,tag=!Ply.Flag.SpJumpAttack,tag=Ply.Ope.IsFlying] run function mhdp_items:weapons/short_sword/type_tec/1_normal_1/start
    execute if entity @s[tag=!Wpn.Ss.Tec.FallSpear,tag=!Ply.Flag.SpJumpAttack,tag=!Ply.Ope.IsFlying] run function mhdp_items:weapons/short_sword/type_tec/19_moveslash/start
