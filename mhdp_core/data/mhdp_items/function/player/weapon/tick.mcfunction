...
# ジャンプ関連処理
    # ベクトルジャンプ
        ...
        execute if entity @s[tag=Ply.Ope.IsKeySprint,tag=Ply.Ope.StartKeyJump,tag=!Ply.Weapon.NoAvoid,tag=!Ply.Ope.IsAir,tag=!Ply.Weapon.StaminaEmpty,tag=!Ply.Weapon.NoMoveJump] unless score @s Ply.Timer.VectorJumpCoolTime matches 1.. run function mhdp_items:player/weapon/move_jump/main
    # ダッシュジャンプ時、ジャンプ回避
