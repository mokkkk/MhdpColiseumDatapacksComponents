...
# 所持している特殊装具に応じてデータを設定
    ...
    # 不動の装衣
        execute if items entity @s container.* minecraft:carrot_on_a_stick[minecraft:custom_data~{IsSpItem:1b,ItemId:1000}] run data modify storage mhdp_core:temp PlayerData.SpItems append value "immovable"
    # 翔蟲
