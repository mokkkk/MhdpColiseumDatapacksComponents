#> mhdp_items:core/util/macro/m.stop_all_animations_loop
# 
# 共通処理 無操作の取得
#
...
# 対象がアニメーションタグの場合、そのタグを消去する
    ...
    $execute if data storage mhdp_core:temp Temp{TagPrefix:"$(TagPrefix)"} run function mhdp_items:core/util/macro/m.stop_all_animations_remove_tag with storage mhdp_core:temp Temp
    data remove storage mhdp_core:temp Temp.TargetTag 
    data remove storage mhdp_core:temp Temp.TagPrefix

# タグが残っている場合、続行
    execute if entity @s[tag=Ply.Weapon.NoOpe] if data storage mhdp_core:temp Temp.Tags[0] run function mhdp_items:core/util/macro/m.stop_all_animations_loop with storage mhdp_core:temp Args
