...
# 効果音
    ...
    execute if score @s aj.fly_tail.frame matches 48..52 run playsound entity.ravager.attack master @a[tag=!Ply.State.IsSilent,distance=..48] ~ ~ ~ 1 0.7 0.4
    execute if score @s aj.fly_tail.frame matches 28 run playsound entity.player.attack.sweep master @a[tag=!Ply.State.IsSilent,distance=..48] ~ ~ ~ 2 0.7
