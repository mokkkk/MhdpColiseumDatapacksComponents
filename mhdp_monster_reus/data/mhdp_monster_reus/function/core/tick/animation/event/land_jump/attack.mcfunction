...
# ヒット確認
    # 対プレイヤー
        execute positioned ^ ^1 ^ as @a[tag=Ply.State.EnableDamage,distance=..5.5] run tag @s add Temp.Hit
        execute as @a[tag=Temp.Hit] run function mhdp_core:player/damage/entity_to_player/main
        tag @a remove Temp.Hit
    # 対モンスター
        execute positioned ^ ^1 ^ as @e[type=slime,tag=Mns.HitBox,tag=!Mns.HitBox.DisableMnsDamage,tag=!Mns.HitBox.Reus,distance=..5.5] run tag @s add Temp.Hit
...
# 風圧怯み
    ...
    execute as @a[tag=Ply.State.EnableDamage,distance=..8] facing entity @s feet positioned as @s run function mhdp_core:player/damage/wind/main
