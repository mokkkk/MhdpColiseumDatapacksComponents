...
# データ読み込み
    data modify storage mhdp_core:temp Damage set from storage mhdp_core:monster_data AttackData[{Uid:1002}].Attacks[{Name:"FlameVertical"}]
...
# 演出
    ...
    execute as @n[type=area_effect_cloud,tag=Mns.ShotPos.Reus] at @s run tp @s ^ ^0.3 ^1
    execute on passengers if entity @s[tag=aj.data] run function mhdp_monster_reus:core/tick/animation/event/fly_flame_vertical/effect/start with entity @s data.locators.pos_head
    execute as @n[type=area_effect_cloud,tag=Mns.ShotPos.Reus] at @s run tp @s ^ ^0.3 ^1
    execute on passengers if entity @s[tag=aj.data] run function mhdp_monster_reus:core/tick/animation/event/fly_flame_vertical/effect/start with entity @s data.locators.pos_head
    execute as @n[type=area_effect_cloud,tag=Mns.ShotPos.Reus] at @s run tp @s ^ ^0.3 ^1
