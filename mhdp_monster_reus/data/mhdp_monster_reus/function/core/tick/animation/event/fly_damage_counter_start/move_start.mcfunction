#> mhdp_monster_reus:core/tick/animation/event/fly_damage_counter_start/move_start
#
...
# ターゲット配置
    execute facing entity @n[tag=Mns.Target.Reus] feet rotated ~ 0 run summon marker ^ ^ ^-4 {Tags:["Temp.Move.Target.Marker"]}
    execute as @e[type=marker,tag=Temp.Move.Target.Marker] at @s run function mhdp_monsters:core/util/other/on_ground

# 移動ベクトル計算
    scoreboard players set #mhdp_temp_move_tick MhdpCore 7
