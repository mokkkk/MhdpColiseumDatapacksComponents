#> mhdp_monster_reus:core/tick/animation/event/fly_tail/attack
#
...
# データ読み込み
    data modify storage mhdp_core:temp Damage set from storage mhdp_core:monster_data AttackData[{Uid:1002}].Attacks[{Name:"FlyTail"}]
...
# ヒット確認
    # 対プレイヤー
        execute on passengers if entity @s[tag=aj.data] run function mhdp_monster_reus:core/tick/animation/event/fly_tail/attack_hit with entity @s data.locators.pos_tail_2
        execute if entity @s[tag=!Mns.Break.Tail] on passengers if entity @s[tag=aj.data] run function mhdp_monster_reus:core/tick/animation/event/fly_tail/attack_hit with entity @s data.locators.pos_tail_3
