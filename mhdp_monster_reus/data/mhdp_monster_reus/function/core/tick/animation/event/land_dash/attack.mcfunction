#> mhdp_monster_reus:core/tick/animation/event/land_dash/attack
#
# アニメーションイベントハンドラ 突進
#
...
# データ読み込み
    data modify storage mhdp_core:temp Damage set from storage mhdp_core:monster_data AttackData[{Uid:1002}].Attacks[{Name:"Dash"}]
...
# ヒット確認
    # 対プレイヤー
        execute positioned ^ ^1 ^2 as @a[tag=Ply.State.EnableDamage,distance=..3] run tag @s add Temp.Hit
        execute as @a[tag=Temp.Hit] run function mhdp_core:player/damage/entity_to_player/main
        tag @a remove Temp.Hit
    # 対モンスター
        execute positioned ^ ^1 ^2 as @e[type=slime,tag=Mns.HitBox,tag=!Mns.HitBox.DisableMnsDamage,tag=!Mns.HitBox.Reus,distance=..3] run tag @s add Temp.Hit
