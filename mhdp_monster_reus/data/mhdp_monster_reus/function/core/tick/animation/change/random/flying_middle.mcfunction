#> mhdp_monster_reus:core/tick/animation/change/random/flying_middle
#
# 行動ランダム選択
#
...
# 確率設定
    data modify storage mhdp_core:temp Temp.AttackRandom set value {FlyMove:3,FlyMoveBreath:3,FlyAssault:3,FlyFlameSweep:3}
    # 正面
...
# 決定
    function mhdp_monster_reus:core/tick/animation/change/random/macro/m.flying_middle with storage mhdp_core:temp Temp.AttackRandom
...
# 飛行ブレス
    execute if score #mndp_temp_action_id MhdpCore matches 2 run function mhdp_monster_reus:core/tick/animation/change/play/fly_movebreath
# 急襲
    execute if score #mndp_temp_action_id MhdpCore matches 3 run tag @s add Anim.Fly.Assault
# なぎはらい火炎放射
...
# 軸合わせ
    ...
    execute if entity @s[tag=Anim.Fly.Assault] run tag @s add Mns.Temp.IsTurn
