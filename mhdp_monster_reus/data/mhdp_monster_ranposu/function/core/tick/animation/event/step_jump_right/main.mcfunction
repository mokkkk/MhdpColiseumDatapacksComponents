...
# 軸合わせ
    execute if score @s aj.step_jump_right.frame matches 1..27 at @s run tag @n[tag=Mns.Target.Ranposu] add Temp.Rotate.Target
    execute if score @s aj.step_jump_right.frame matches 1..27 at @s run function mhdp_monsters:core/util/other/turn_to_target_accurate

# 移動
    execute if score @s aj.step_jump_right.frame matches 1..18 at @s run tp @s ^-1 ^ ^
    execute if score @s aj.step_jump_right.frame matches 28 at @s run function mhdp_monster_ranposu:core/tick/animation/event/step_jump_right/move_start
    execute if score @s aj.step_jump_right.frame matches 30..44 at @s run function mhdp_monsters:core/util/other/move_to_target_move
...
# 攻撃
    execute if score @s aj.step_jump_right.frame matches 32..44 run function mhdp_monster_ranposu:core/tick/animation/event/step_jump_right/attack
