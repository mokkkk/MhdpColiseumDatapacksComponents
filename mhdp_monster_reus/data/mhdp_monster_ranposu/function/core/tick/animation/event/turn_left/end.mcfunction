#> mhdp_monster_ranposu:core/tick/animation/event/turn_left/end
#
...
# 軸合わせ回数が残っており、まだ角度が付いている場合、再度軸合わせを行う
    execute facing entity @n[tag=Mns.Target.Ranposu] feet rotated ~ 0 positioned ^ ^ ^1 rotated as @s positioned ^ ^ ^-1 if entity @s[distance=..0.25] at @s run tag @s add Mns.Temp.NoTurn
