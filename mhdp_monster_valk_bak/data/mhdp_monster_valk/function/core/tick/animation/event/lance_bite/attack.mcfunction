...
# データ読み込み
    data modify storage mhdp_core:temp Damage set from storage mhdp_core:monster_data AttackData[{Uid:1004}].Attacks[{Name:"Bite"}]
...
# ヒット確認
    # 対プレイヤー
        execute positioned ^ ^1 ^4 as @a[tag=Ply.State.EnableDamage,distance=..3.8] run tag @s add Temp.Hit
