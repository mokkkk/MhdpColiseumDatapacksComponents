...
execute if score @s aj.death.frame matches 176.. run scoreboard players set @s aj.death.frame 1
data remove storage animated_java:temp args
execute store result storage animated_java:temp args.frame int 1 run scoreboard players get @s aj.death.frame
execute at @s run function animated_java_dino:dino/animations/death/zzz/apply_frame with storage animated_java:temp args
