...
# 部位破壊
    execute if entity @s[tag=!Mns.Break.LegL] if score @s Mns.Dino.LegL.Damage.Count matches 1.. run function mhdp_monster_dino:core/damage/break/leg_r
...
# アニメーション再生
    execute if entity @s[tag=!Mns.State.IsFlying,tag=!Mns.Temp.IsDamaged] unless score @s Mns.Dino.LegL.Damage.Count matches 2.. run function animated_java_dino:dino/animations/damage_right/tween {duration:1, to_frame: 0}
    execute if entity @s[tag=!Mns.State.IsFlying] if score @s Mns.Dino.LegL.Damage.Count matches 2.. run function animated_java_dino:dino/animations/damage_down_right/tween {duration:1, to_frame: 0}
