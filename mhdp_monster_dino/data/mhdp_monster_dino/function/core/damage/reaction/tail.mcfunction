...
# アニメーション再生
    ...
    execute if entity @s[tag=!Mns.State.IsFlying,tag=!Mns.Temp.IsDamaged,tag=!Mns.Temp.IsDamage.TailCut] if entity @n[tag=Temp.Attacker,tag=Mns.Temp.Situation.IsForward] run function animated_java_dino:dino/animations/damage_tail_forward/apply_frame {duration:1, to_frame: 0}
    execute if entity @s[tag=!Mns.State.IsFlying,tag=!Mns.Temp.IsDamaged,tag=!Mns.Temp.IsDamage.TailCut] if entity @n[tag=Temp.Attacker,tag=!Mns.Temp.Situation.IsForward] run function animated_java_dino:dino/animations/damage_tail_back/apply_frame {duration:1, to_frame: 0}
