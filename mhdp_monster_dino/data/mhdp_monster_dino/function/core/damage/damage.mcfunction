...
# 怯み適用：優先度順
    ...
    # # 大ダウン(独自)
    #     execute unless entity @s[tag=!Mns.Temp.IsDamaged,tag=!Mns.Temp.Damage.Head,tag=!Mns.Temp.Damage.Body] if score @s Mns.Dino.DamageCount matches 2.. run function mhdp_monster_dino:core/damage/reaction/sp
    # # 頭(独自)
