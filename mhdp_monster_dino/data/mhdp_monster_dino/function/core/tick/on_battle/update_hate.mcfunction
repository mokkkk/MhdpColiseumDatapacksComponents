...
# プレイヤーの状態取得
    function mhdp_monsters:core/util/tick/fetch_player/check_player_situation.m {Tag:"Mns.Candidate.Ranposu",DistNear:12,DistFar:30,Fov:90.0}

# 警戒度更新
    # 近距離
        execute as @a[tag=Mns.Temp.Situation.Near] run scoreboard players add @s Mns.Ranposu.Hate 8
    # 中距離
...
# 別エリアのプレイヤーの発見度、ヘイト減少
    execute as @a[tag=Ply.State.PlayingQuest,tag=!Mns.Candidate.Ranposu,scores={Mns.Ranposu.Hate=1..}] run scoreboard players remove @s Mns.Ranposu.Hate 1
