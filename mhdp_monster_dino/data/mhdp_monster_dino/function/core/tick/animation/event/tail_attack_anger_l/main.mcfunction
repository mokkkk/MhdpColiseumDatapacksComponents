...
# 弱めの軸合わせ
    execute if score @s aj.tail_attack_anger_l.frame matches 2 run function mhdp_monster_dino:core/tick/animation/event/tail_attack_anger_l/turn_start
...
# 状態更新
    execute if score @s aj.tail_attack_anger_l.frame matches 18 run tag @s add Mns.Dino.State.Attack.Tail.L
    execute if score @s aj.tail_attack_anger_l.frame matches 90 run tag @s remove Mns.Dino.State.Attack.Tail.L
