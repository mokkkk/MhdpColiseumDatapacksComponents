...
# 移動
    execute if score @s aj.tail_back_l.frame matches 31 at @s run function mhdp_monsters:core/util/tick/event/vector_move_offset_start.m {\
        Tick:10,OffsetX:0.0,OffsetY:0.0,OffsetZ:6.0,IsAdjustLand:"true"\
    }
    execute if score @s aj.tail_back_l.frame matches 34..43 at @s run function mhdp_monsters:core/util/tick/event/vector_move
...
# 攻撃
    execute if score @s aj.tail_back_l.frame matches 20 run tag @s remove Mns.Temp.Attack.Tail.L
    execute if score @s aj.tail_back_l.frame matches 36 run function mhdp_monsters:core/util/tick/event/start_attack.m with storage mhdp_core:monster_data AttackData[{Uid:1003}].Attacks[{Name:"TailBack"}]
    execute if score @s aj.tail_back_l.frame matches 37..44 at @s run function mhdp_monster_dino:core/tick/animation/event/tail_back_l/attack_tail_start
    execute if score @s aj.tail_back_l.frame matches 40 run function mhdp_monsters:core/util/tick/event/end_attack
