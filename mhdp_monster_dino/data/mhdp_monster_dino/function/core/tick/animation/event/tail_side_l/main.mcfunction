...
# 軸合わせ・弱め
    execute if score @s aj.tail_side_l.frame matches 2 run function mhdp_monsters:core/util/tick/event/alignment_start.m {TargetTag:"Mns.Target.Dino",Tick:10,MaxRotation:180}
    execute if score @s aj.tail_side_l.frame matches 10 run function mhdp_monsters:core/util/tick/event/alignment_start.m {TargetTag:"Mns.Target.Dino",Tick:10,MaxRotation:180}
    execute if score @s aj.tail_side_l.frame matches 2..18 at @s run function mhdp_monsters:core/util/tick/event/alignment
...
# 攻撃
    execute if score @s aj.tail_side_l.frame matches 31 run tag @s add Mns.Temp.Attack.Tail.L
    ...
    execute if score @s aj.tail_side_l.frame matches 71 at @s run function mhdp_monster_dino:core/tick/animation/event/tail_side_l/particle_tail_start
    execute if score @s aj.tail_side_l.frame matches 40 run function mhdp_monsters:core/util/tick/event/end_attack

# 接地
    function mhdp_monsters:core/util/tick/move/check_landing

# フェーズ
    execute if entity @s[tag=!Mns.Dino.State.TailHeat] if score @s aj.tail_side_l.frame matches 32 run function mhdp_monster_dino:core/util/phase/tail_heat_check
