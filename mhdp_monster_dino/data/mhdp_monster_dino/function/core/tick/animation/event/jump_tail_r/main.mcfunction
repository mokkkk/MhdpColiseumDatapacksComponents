...
# 弱めの軸合わせ
    ...
    execute if score @s aj.jump_tail_r.frame matches 30..35 at @s run function mhdp_monsters:core/util/other/turn_to_target_rotate

# 移動
    execute if score @s aj.jump_tail_r.frame matches 1..5 if entity @n[tag=Mns.Target.Dino,distance=..8] at @s run tp @s ^ ^ ^-0.2
    execute if score @s aj.jump_tail_r.frame matches 23 at @s run function mhdp_monster_dino:core/tick/animation/event/jump_tail_r/move_start
    execute if score @s aj.jump_tail_r.frame matches 24..37 at @s run function mhdp_monsters:core/util/other/move_to_target_move
...
# 状態更新
    execute if score @s aj.jump_tail_r.frame matches 30 run tag @s add Mns.Dino.State.Attack.Tail.R
    execute if score @s aj.jump_tail_r.frame matches 50 run tag @s remove Mns.Dino.State.Attack.Tail.R
