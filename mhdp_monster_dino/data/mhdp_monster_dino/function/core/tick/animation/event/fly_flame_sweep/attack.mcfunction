#> mhdp_monster_reus:core/tick/animation/event/fly_flame_sweep/attack
#
...
# 演出
    execute on passengers if entity @s[tag=aj.data] run function mhdp_monster_reus:core/tick/animation/event/fly_flame_sweep/effect/start with entity @s data.locators.pos_head
    execute as @n[type=area_effect_cloud,tag=Mns.ShotPos.Reus] at @s run tp @s ^-1 ^ ^
    execute on passengers if entity @s[tag=aj.data] run function mhdp_monster_reus:core/tick/animation/event/fly_flame_sweep/effect/start with entity @s data.locators.pos_head
    execute as @n[type=area_effect_cloud,tag=Mns.ShotPos.Reus] at @s run tp @s ^-1 ^ ^
