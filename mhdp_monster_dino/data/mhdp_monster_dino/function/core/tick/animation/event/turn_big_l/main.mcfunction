...
# 軸合わせ
    execute if score @s aj.turn_big_l.frame matches 5 run function mhdp_monsters:core/util/tick/event/alignment_start.m {TargetTag:"Mns.Target.Dino",Tick:21,MaxRotation:360}
    execute if score @s aj.turn_big_l.frame matches 5..25 at @s run function mhdp_monsters:core/util/tick/event/alignment

# 移動
    execute if score @s aj.turn_big_l.frame matches 1..18 facing entity @n[tag=Mns.Target.Dino] feet rotated ~ 0 run tp @s ^ ^ ^-0.2
