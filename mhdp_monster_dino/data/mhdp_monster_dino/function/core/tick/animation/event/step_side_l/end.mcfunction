...
# それ以外の場合、噛みつき
    execute if entity @s[tag=!Mns.State.IsAnger,tag=!Mns.Temp.IsCombo] if entity @n[tag=Mns.Target.Dino,distance=..11] run tag @s add Anim.Bite
    execute if entity @s[tag=Mns.State.IsAnger,tag=!Mns.Temp.IsCombo] if entity @n[tag=Mns.Target.Dino,distance=..11] run tag @s add Anim.BiteToTail
