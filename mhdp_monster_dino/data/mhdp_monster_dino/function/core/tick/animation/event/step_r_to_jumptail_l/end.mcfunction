...
# 尻尾たたきつけに移行
    ...
    execute if entity @s[tag=!Mns.State.IsAnger] run function animated_java_dino:dino/animations/jump_tail_l/tween {duration:1, to_frame: 10}
    execute if entity @s[tag=Mns.State.IsAnger] run function animated_java_dino:dino/animations/jump_tail_anger_l/tween {duration:1, to_frame: 10}
