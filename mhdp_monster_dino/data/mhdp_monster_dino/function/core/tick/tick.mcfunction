...
# メイン処理
    function mhdp_monster_dino:core/tick/main

# 怒り中、一定時間ごとにメイン処理を再度実行
    execute if entity @s[tag=Mns.State.IsAnger] run scoreboard players add @s Mns.Temp.AngerSpeed.Timer 1
    execute if entity @s[tag=!Mns.State.IsDisableAngerSpeed] if score @s Mns.Temp.AngerSpeed.Timer >= @s Mns.Anger.Speed at @s run function mhdp_monster_dino:core/tick/main
