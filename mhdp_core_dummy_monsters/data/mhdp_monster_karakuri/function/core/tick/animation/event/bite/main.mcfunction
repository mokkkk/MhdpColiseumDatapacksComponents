...
# 攻撃
    execute if score @s aj.bite.frame matches 41 run function mhdp_monsters:core/util/tick/event/start_attack.m with storage mhdp_core:monster_data AttackData[{Uid:8001}].Attacks[{Name:"Bite"}]
    execute if score @s aj.bite.frame matches 45 run function mhdp_monster_karakuri:core/tick/animation/event/bite/attack
    execute if score @s aj.bite.frame matches 46 run function mhdp_monsters:core/util/tick/event/end_attack
