...
# ヒット確認
    # 対プレイヤー
        execute positioned ^ ^1 ^6.7 as @a[tag=Ply.State.EnableDamage,distance=..2.5] run tag @s add Temp.Hit
        execute as @a[tag=Temp.Hit] run function mhdp_core:player/damage/entity_to_player/main
        tag @a remove Temp.Hit
    # 対モンスター
        execute positioned ^ ^1 ^6.7 as @e[type=slime,tag=Mns.HitBox,tag=!Mns.HitBox.DisableMnsDamage,tag=!Mns.HitBox.Karakuri,distance=..2.5] run tag @s add Temp.Hit
